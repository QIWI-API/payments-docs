...
Чек передается в необязательном JSON объекте `cheque` в операциях [выставления счета ](#invoicing_requests) и [платежа](#api_requests).
...
В тело [платежного запроса](#payments) добавляется JSON-объект `paymentSplits`, используемый для передачи данных о поставщиках.
...
Поле | Тип | Описание
----|------|-------
paymentSplits | Array | Параметр для передачи данных о поставщиках
-----|------|------
type | String | Тип передаваемых данных. Доступные параметры - `MERCHANT_DETAILS` (данные поставщика)
...
-----|----|-----
merchantCms | Object | Данные о комиссии поставщика
----|-----|----
value | Number | Сумма комиссии
currency | Number | Код валюты комиссии
-----|-----|-----
...
<aside class="warning">Токен выпускается только после прохождения 3DS-аутентификации клиентом и успешной авторизации платежа банком-эмитентом, то есть в ответ на запрос завершения аутентификации клиента <a href="#payment_complete">COMPLETE</a></aside>
