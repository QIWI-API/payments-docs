...
Когда интеграция на вашей стороне закончена, мы переводим `siteId` в боевой режим. В боевом режиме выполняются реальные списания с карт.
...
[Подробнее о возможных параметрах запроса](#invoice_put)
...
payUrl|String| [Ссылка](#http_oplata) на созданную платежную форму
...
[Подробнее о возможных параметрах запроса](#invoice_capture)


## Запросы, Статусы и Ошибки {#invoicing_requests}


<aside class="notice">В запросы и ответы могут добавляться дополнительные поля. Следите за обновлениями на <a href="https://github.com/QIWI-API/payments-docs/blob/master/payments_ru.html.md">Github</a>.</aside>
...
[Подробнее о возможных параметрах запроса](#payments)
...
[Подробнее о возможных параметрах запроса](#payment_complete)
...
[Подробнее о возможных параметрах запроса](#capture)
...
Укажите адрес сервера для обработки уведомлений в Личном Кабинете на сайте <a href="https://kassa.qiwi.com/">kassa.qiwi.com</a> в разделе "Настройки". Можно также указывать адрес сервера в опциональном параметре `callbackUrl` в запросах к [API](#api_requests).
...
Чек передается в необязательном параметре `cheque` в операциях [выставления счета ](#invoicing_requests) и [платежа](#api_requests) при работе через Checkout и API, соответственно.
...
**Протокол онлайн платежей** поддерживает механизм токенизации карт пользователей. Токенизация карты позволяет сохранять карточные данные в зашифрованной строке платежного токена и в дальнейшем использовать его для безакцептных платежей.
...
### Если вы используете [Платежную форму](#invoicing)
...
<aside class="warning">
Не указывайте один и тот же параметр <code>account</code> для всех ваших пользователей. В результате пользователи смогут произвести оплату чужими картами.
</aside>
...
* обязательно укажите параметры токена в объекте `paymentMethod` вместо карточных данных и идентификатор покупателя:
