...
* объект `external3dSec` с информацией из расшифрованного платежного токена Apple:
   * `type` — всегда `APPLE_PAY`;
   * `onlinePaymentCryptogram` — содержимое поля `onlinePaymentCryptogram` (Base64-закодированная строка);
...
Отправьте платежные данные в QIWI. Для этого передайте в запросе API [Платеж](#payments) объект `paymentMethod` с параметрами: 
...
> Пример платежа с данными расшифрованного платежного токена Google Pay
...
* `type` — всегда `CARD`;
* объект `external3dSec` с информацией из расшифрованного платежного токена Google:
   * `type` — всегда `GOOGLE_PAY`;
   * `cryptogram` — содержимое поля `cryptogram` (Base64-закодированная строка);
...
После оплаты платеж перейдет в финальный статус и будет отправлено [уведомление](#payment-callback). 
