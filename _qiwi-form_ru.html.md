...
При оплате счета, выставленного таким способом, платеж автоматически авторизуется без участия мерчанта. Подключите автоматическое подтверждение платежа в Службе поддержки, чтобы не было необходимости выполнять запрос [Подтверждение платежа](#capture).
...
<aside class="warning">
На Платежной форме QIWI по умолчанию покупателю доступен только способ оплаты банковской картой. Для подключения способов оплаты "QIWI Кошелек", "Apple Pay/Google Pay", "Оплата платежным токеном" обратитесь к вашему сопровождающему менеджеру.
</aside>
...
1. Чтобы выставить счет для оплаты через холдирование средств на карте (двухшаговый платеж), передайте в запросе API [Создание счета](#invoice_put):
...
3. Дождитесь завершения платежа: вам придет [уведомление](#payment-callback), или периодически отправляйте запрос [Статус платежа](#invoice_get), чтобы получить информацию о платеже.
...
1. Передайте в запросе API [Создание счета](#invoice_put) следующие данные:
   * ключ API;
   * сумму счета (`amount`);
   * дату, до которой необходимо оплатить счет (`expirationDateTime`);
