...
activate store
store->>qb:Выставление счета<br/>Одношаговый сценарий — все способы оплаты<br/>Двухшаговый сценарий — только карты и КИВИ Кошелек
activate qb
qb->>store:Ссылка на платежную форму QIWI (payUrl)
store->>customer:Переадресация покупателя на payUrl
customer->>qb:Открытие платежной формы,<br/>выбор способа оплаты,<br/>ввод платежных реквизитов
qb->>customer:Аутентификация покупателя:<br/>3DS — карты, QW — авторизация в КИВИ кошельке
...
При оплате счета, выставленного таким способом, платеж авттоматически авторизуется без участия мерчанта. Подключите автоматическое подтверждение платежа в Службе поддержки, чтобы не было необходимости выполнять запрос [Завершение аутентификации](#capture).
...
>Пример выставления счета с оплатой через холдирование (двухшаговый сценарий)
...
>Пример выставления счета с оплатой без авторизации Покупателя (одношаговый сценарий)
...
Чтобы выставить счет с оплатой через холдирование средств на карте (двухшаговый сценарий), передайте в запросе API [Создание счета](#invoice_put):
...
Для оплаты счета без авторизации покупателя (одношаговый сценарий) укажите в запросе API [Создание счета](#invoice_put) параметр `"flags":["SALE"]`. Если не передать этот параметр, то будет выполнено безусловное холдирование средств для оплаты счета.
...
* `customer.account` — уникальный идентификатор Покупателя в системе ТСП. **Указывайте разные параметры <code>account</code> для разных покупателей, чтобы гарантировать безопасность карточных данных покупателей.**
...
Чтобы выставить счет с оплатой платежным токеном, передайте в запросе API [Создание счета](#invoice_put):
