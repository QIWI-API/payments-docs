...
5. Дождитесь завершения платежа: вам придет [уведомление](#capture-callback), или периодически отправляйте запрос API [Статус подтверждения](#capture_get), чтобы получить информацию о платеже.
...
Подробнее о выпуске платежного токена см. в [соответствующем разделе](#payment-token-issue).
