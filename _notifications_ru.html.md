...
Тип уведомления указан в поле `type` тела уведомления.
...
Поле|Описание|Тип|В каких уведомлениях
--------|--------|---|------
...
type| [Тип операции](#operation-types) |String(200)| Во всех уведомлениях
createdDatetime | Дата создания операции | URL-закодированная строка<br>`ГГГГ-ММ-ДДTЧЧ:ММ:ССZ`|Во всех уведомлениях
amount | Информация о сумме операции |Object|Во всех уведомлениях
--|--|----|---
...
---|---|---|----
billId| ID счета, соответствующего операции| String(200)|Во всех уведомлениях
status | Информация о статусе операции | Object|Во всех уведомлениях
---|---|---|------
...
rrn| RRN платежа (по ISO 8583)| Number|Во всех уведомлениях
authCode| Auth-code платежа| Number|Во всех уведомлениях
paymentCardInfo | Информация о карте.| Object |  В уведомлениях PAYMENT
---|---|---|-------
issuingCountry | Код страны эмитента | String(3)|В уведомлениях PAYMENT
issuingBank | Банк-эмитент | String|В уведомлениях PAYMENT
paymentSystem | Тип платежной системы | String|В уведомлениях PAYMENT
fundingSource | Тип карты | String |В уведомлениях PAYMENT
paymentSystemProduct | Категория карты | String|В уведомлениях PAYMENT
---|---|---|-----
...
---|---|---|-----
paymentSplits | Описание сплитованных платежей. | Array(Objects) | В уведомлениях PAYMENT по [сплитованным платежам](#split-payments-board)
-----|------|------|-----
type | Тип передаваемых данных. Всегда строка `MERCHANT_DETAILS` | String|В уведомлениях PAYMENT по [сплитованным платежам](#split-payments-board)
siteUid | [ID поставщика](#start) | String|В уведомлениях PAYMENT по [сплитованным платежам](#split-payments-board)
splitAmount | Данные о возмещении поставщику | Object|В уведомлениях PAYMENT по [сплитованным платежам](#split-payments-board)
----|----|-----|-----
value | Сумма возмещения | Number|В уведомлениях PAYMENT по [сплитованным платежам](#split-payments-board)
currency | Буквенный код валюты возмещения по ISO | String(3)|В уведомлениях PAYMENT по [сплитованным платежам](#split-payments-board)
----|----|-----|-----
splitCommissions | Данные о комиссии | Object|В уведомлениях PAYMENT по [сплитованным платежам](#split-payments-board)
-----|----|-----|-----
merchantCms | Данные о комиссии с поставщика | Object|В уведомлениях PAYMENT по [сплитованным платежам](#split-payments-board)
----|-----|----|-----
value | Сумма комиссии | Number|В уведомлениях PAYMENT по [сплитованным платежам](#split-payments-board)
currency |Буквенный код валюты комиссии по ISO | String(3)|В уведомлениях PAYMENT по [сплитованным платежам](#split-payments-board)
-----|-----|-----|-----
orderId | Номер заказа | String|В уведомлениях PAYMENT по [сплитованным платежам](#split-payments-board)
comment | Комментарий к заказу | String|В уведомлениях PAYMENT по [сплитованным платежам](#split-payments-board)
-----|-----|-----|-----
refundSplits | Описание возвратов по сплитованным платежам. | Array(Objects) | В  уведомлениях REFUND по возвратам [сплитованных платежей](#split-payments-board)
-----|------|------|-----
type | Тип передаваемых данных. Всегда строка `MERCHANT_DETAILS` | String|В  уведомлениях REFUND по возвратам [сплитованных платежей](#split-payments-board)
siteUid | [ID поставщика](#start) | String|В  уведомлениях REFUND по возвратам [сплитованных платежей](#split-payments-board)
splitAmount | Данные об отмене возмещения поставщику | Object|В  уведомлениях REFUND по возвратам [сплитованных платежей](#split-payments-board)
----|----|-----|-----
value | Сумма отмены возмещения | Number|В  уведомлениях REFUND по возвратам [сплитованных платежей](#split-payments-board)
currency | Буквенный код валюты отмены возмещения по ISO | String(3)|В  уведомлениях REFUND по возвратам [сплитованных платежей](#split-payments-board)
----|----|-----|-----
splitCommissions | Данные о комиссии | Object|В  уведомлениях REFUND по возвратам [сплитованных платежей](#split-payments-board)
-----|----|-----|-----
merchantCms | Данные о комиссии с поставщика | Object|В  уведомлениях REFUND по возвратам [сплитованных платежей](#split-payments-board)
----|-----|----|-----
value | Сумма комиссии | Number|В  уведомлениях REFUND по возвратам [сплитованных платежей](#split-payments-board)
currency |Буквенный код валюты комиссии по ISO | String(3)|В  уведомлениях REFUND по возвратам [сплитованных платежей](#split-payments-board)
-----|-----|-----|-----
orderId | Номер заказа | String|В  уведомлениях REFUND по возвратам [сплитованных платежей](#split-payments-board)
comment | Комментарий к заказу | String|В  уведомлениях REFUND по возвратам [сплитованных платежей](#split-payments-board)
-----|-----|-----|-----
