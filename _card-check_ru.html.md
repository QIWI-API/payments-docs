...
4. Дождитесь завершения проверки: вам придет [уведомление CHECK_CARD](#check-card-callback) с результатом. Информация о доступности карты для списаний содержится в атрибуте `isValidCard` (`true` — номер карты валиден). Данные платежного токена возвращаются в объекте `createdToken` уведомления.
