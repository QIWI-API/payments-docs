...
<aside class="warning">
Способ взаимодействия "Платеж через форму мерчанта" допускается только в случае PCI DSS сертификации, т.к. прием и обработка карточных данных покупателей выполняется на стороне мерчанта.
</aside>
...
`*` — посредством выпуска платежного токена для QIWI кошелька
