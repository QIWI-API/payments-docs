...
###### Последнее обновление: 2021-03-11 | [Редактировать на GitHub](https://github.com/QIWI-API/payments-docs/blob/master/payments_ru.html.md)
...
**MPI**: Merchant Plug-In — модуль, выполняющий 3DS аутентификацию.  
...
При подключении ваш идентификатор находится в тестовом режиме. В этом режиме вы можете проводить  операции без списания средств с банковской карты. Подробнее о тестовом режиме см. в разделе [Тестовый и производственный режим](#test_mode).
...
<aside class="warning">
Способ взаимодействия "Платеж через форму ТСП" допускается только в случае PCI DSS сертификации ТСП, т.к. ТСП выполняет прием и обработку карточных данных покупателей.
</aside>

## Доступные методы оплаты {#payment-ways}

Метод|Платежная форма QIWI|Платежная форма ТСП
-----|---------|--------------
Банковская карта|+|+
...
## Общая схема проведения платежа и взаиморасчетов {#principial-scheme}


<div class="mermaid">
sequenceDiagram
participant customer as Покупатель
participant store as Магазин ТСП
participant ipsstore as Кредитная организация <br/> ТСП
...
Для аутентификации запросов используется стандарт OAuth 2.0 согласно [RFC 6750](https://tools.ietf.org/html/rfc6750). Указывайте значение ключа доступа к API в HTTP-заголовке `Authorization` как 
...
**При переходе в производственный режим перевыпускать ключ доступа к API не нужно. Однако если вы меняете постоянный URL для уведомлений с тестового (например, https://your-shop-test.ru/callbacks) на производственный (например, https://your-shop-prod.ru/callbacks), то необходимо выпустить новый ключ API**.
